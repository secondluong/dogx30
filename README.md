...
| `tools/config_test.sh` | 在线改配置测试：令牌、校验、落盘、自重启 |
...
装完之后这些参数在控制台右上角的「设置」里就能改：运动主机、感知主机、
监听地址与端口、点云开关与它那几个参数。改完网关自己重启，一两秒后恢复。

现场真正需要这个的场合是：换了一台感知主机、或者发现板子被尾部那个路由器
NAT 到了别的网段要改绑定地址。蹲在狗旁边用 `vi` 编辑 systemd 单元不现实。
...
令牌文件是 `600 root`，所以要 `sudo`——不加会提示你加。

为什么单设这道门：协议没有身份认证（见下），而改配置能把网关指到另一台主机上，
也能把监听地址从内网改成 `0.0.0.0` 自己把口子开出来——这比操控狗更值得拦一道。
令牌是装机时生成的，重装不会换。

两件事会被拒绝，都是有意的：

- **有人正持有控制权时不许改。** 改配置要重启，重启会中断遥控。
  狗正走着的时候不能发生，先让对方释放。
- **监听地址填成本机没有的地址时不许改。** 这条最要紧：配错了重启后
  `bind` 失败，服务起不来，控制台跟着消失，就再没有地方能改回来。开点云时
  `ros_host` 同理。

参数存在 `/opt/x30/conf/gateway.conf`，手工改也行，改完
`systemctl restart x30-gateway`。**systemd 单元里刻意不再重复这些参数**，
只指向这个文件——两处各存一份的话，迟早出现「单元里写着 `.103`、实际跑的是
`.200`」，而去读单元的人会信单元。

升级时不带参数重跑 `install.sh` 不会冲掉在控制台改过的值；
显式给了 `--robot-ip` 之类的参数则以命令行为准，并写回配置文件。
...
唯一要凭证的操作是[在线改配置](#改配置不用上命令行)，因为它能把监听面
自己打开，放任不管等于让 `--bind` 这条限制形同虚设。其余消息一律不校验身份。

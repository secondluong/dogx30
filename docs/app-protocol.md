...
**不会向狗发任何指令**。记得站着之后摇杆就能走，不必再等力控/起步。
...
界面上点「实心 / 格栅 / 无踢面」会带上这个字段再发一次楼梯步态，不能只改按钮高亮。
...
非楼梯步态在人没按起步时**只记下、不发给主机**，摇杆也不会把记下的档
冲出去。主机文档写步态仅踏步态可切；RL 站着立刻下发会自己踏步，再切
档也停不掉。点这些档会发力控停踏步，界面改走姿态通道，避免 20Hz 速度
0 把狗钉在原地踏。楼梯仍等遥测确认，失败才报 `gait_not_applied`。
...
```json
{"t":"hello","version":"0.2.13","client_id":3,"control":false,"lease_ms":2000,

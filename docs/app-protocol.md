...
水炮（不需要运动控制权，与云台同属载荷）。侦检模式下小摇杆走上面的 `ptz`；
水炮模式下左小是炮台水平/俯仰，右小拨住是雾/柱花型，HOME 按住开阀出水。
网关把这三条合成消防炮 13 字节 TCP 帧，连 `cannon_ip:cannon_port`
（默认 `192.168.1.253:4000`，板子 eth0 去连）。持续指令约 100 ms 一次；
回中、松开发射、急停都发停止帧。400 ms 没新指令也停。

```
{"t":"cannon","pan":0.4,"tilt":-0.2}
{"t":"cannon_spray","value":"fog"}    // 或 jet / mid
{"t":"cannon_fire","on":true}         // HOME 按下；松开 on=false
```

`state` 里可带 `cannon`：`online` / `fire` / `spray` / 角度与压力。

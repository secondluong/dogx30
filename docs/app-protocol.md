...
**不会向狗发任何指令**。记得站着之后仍要人力控或起步，摇杆才有通道。
...
分别对应实心踏面、格栅、无踢面，缺省 `solid`。踏面按钮只记材质；
点「楼梯」步态时才带上这个字段下发。选错的表现是楼梯切不过去。
...
越野 28cm）。只有当前已是楼梯或非手动时才补发手动模式。

非楼梯步态在人没按起步时**只记下、不发给主机**，也不发力控。主机文档
写步态仅踏步态可切；RL 站着立刻下发会自己踏步，一点力控又会把摇杆从
走路拧成调姿。踏面按钮只记材质；点「楼梯」才走地形图流程，没切成会
立刻拉回手动。楼梯失败才报 `gait_not_applied`。
...
云台（不需要运动控制权；载荷与运动链路独立）。侦检模式下左小 CH13/CH14 是水平俯仰，右小 CH16 是变倍，CH15 左右切画中画。
各轴 -1..1：`pan` 右为正，`tilt` 上为正，`zoom` 拉近为正。
网关转成球机 `/cgi-bin/anv/ptz_cgi`（账号默认 admin/admin 的 MD5），不是海康 ISAPI。
App 在能直达球机时也会自己发同一条 CGI。2.4G 运动档 yield 掉 claim 后，云台仍走 MESH/CGI。
...
水炮（不走 MESH）。炮台在 1 网。App 把摇杆、雾/柱、HOME 开阀合成 13 字节 TCP 帧，
绑在 2.4G 网卡上先直连 `192.168.1.253:4000`。现场平板源地址到得了狗、到不了炮台，
直连失败后改连网关 `192.168.1.120:4001`，由网关 eth0 转到炮台。
左小是水平/俯仰，右小拨住是雾/柱，HOME 按住开阀。持续指令约 100 ms 一次；
回中、松开发射发停止帧。水压从炮台回传的 `08 00 00 03` 帧解析，大于 0 时顶栏显示。

```
水炮不进 WebSocket。下面三条只作字段说明，App 交给原生 radioCannon：
pan / tilt   -1..1，右为正、上为正
spray        fog / jet / 空
fire         HOME 按下为 true，松开为 false
```
...
| `ptz_vis_rtsp` | 字符串 | 双光布控球一路拼接画面的 RTSP。默认 `rtsp://192.168.10.168:554/11`。云台口令从地址里取主机 |
| `ptz_vis_codec` / `ptz_ir_codec` | 字符串 | `h264` 或 `h265`。可空：路径含 `/h264`、`/h265` 时按地址猜；`/11` 这类短路径按 `h264`（避免沿用 media.json 的 h265 把旧 WebView 挡掉） |
| `cannon_ip` / `cannon_port` | 字符串 / 数字 | 消防炮 TCP Server。默认 `192.168.1.253:4000`。空 IP 则不连 |
...
- **别人正持有控制权** → `busy_control`。改配置要重启网关，遥控会中断一两秒，
  狗正走着的时候不能发生。自己占着控制权（App 连上会自动 claim）仍可保存。
...
```json
{"t":"hello","version":"0.2.21","client_id":3,"control":false,"lease_ms":2000,
...
**运动与载荷分流**：2.4G 档运动只走射频，MESH 档运动只走网关；视频、云台、水炮、
开关、点云等能走 MESH 的载荷不跟运动档位切断——2.4G 下仍连着网关 WebSocket。
...
以狗为原点的毫米值，下发前换成米。`tags` 里是见过的标签，ID 原样带上，不按名单过滤。
...
串口服务器（`payload_ip:payload_port`，现场 `192.168.10.201:2000`）。
